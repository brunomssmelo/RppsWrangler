...
#' @return um objeto do tipo data.frame contendo o resumo da política de investimentos do RPPS.
#' @author Bruno M. S. S Melo
#' @details
#' A funcao extrai a tabela contendo o resumo da alocacao de recursos definida no "DPIN".
#' @examples
#' mtpsImportaDpin("dpin.pdf")
...
mtpsImportaDpin <- function(arqDpin) {
  ...
  tokens <- str_replace_all(iconv(enc2native(fieldNames), to = "ASCII//TRANSLIT"), "\"", "")
  ...
  fullText <- str_replace_all(iconv(enc2native(fullText), to = "ASCII//TRANSLIT"), "\"", "")
    ...
    try({
      ...
      for (i in 1:length(posTokens)) {
        ...
        if (hasToken[i]) {
          ...
          strChunk <- str_sub(fullText,posTokens[[i]][2],nchar(fullText))
          ...
          dfDpin <- rbind(
            ...
            data.frame(
              ...
              TIPO = tokens[i],
              LIMITE_RESOLUCAO = as.numeric(str_replace(str_sub(strChunk, valPos[[1]][1,1], valPos[[1]][1,2]),",",".")),
              ESTRATEGIA_ALOCACAO = as.numeric(str_replace(str_sub(strChunk, valPos[[1]][2,1], valPos[[1]][2,2]),",","."))
            )
          )
        }
      }
